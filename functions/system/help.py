    ...
    async def show_help(self, interaction: Interaction, command_name: Optional[str] = None):
        if command_name is None:
            ...
            for cmd in self.bot.tree.get_commands():
                embed.add_field(name=cmd.name, value=cmd.description or "No description", inline=False)
            ...
            if command:
                ...
                await interaction.response.send_message(f":x: Command `{command_name}` not found.", ephemeral=True)
